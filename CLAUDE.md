...
- **`lua/plugins/avante.lua`** — Avante.nvim chat, **Ollama-only** (offline, small `llama3.2:1b` model): `<leader>aa` open with current provider, `<leader>ao` (re)select ollama and open. The Claude/Anthropic provider is intentionally disabled (subscription-OAuth ToS risk); re-enable via an API key per the avante guide if needed.
