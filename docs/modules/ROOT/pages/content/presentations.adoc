...
[cols="1,3"]
|===
| xref:#quick-start[Quick Start] | Start server and preview slides
| xref:#starting-server[Starting the Preview Server] | Docker Compose commands
| xref:#keybindings[Keybindings] | Preview and export keys
| xref:#commands[User Commands] | Available Neovim commands
| xref:#how-it-works[How It Works] | Implementation details
| xref:#configuration[Configuration] | Port and path settings
| xref:#marp-syntax[MARP Markdown Syntax] | Slide directives
| xref:#prerequisites[Prerequisites] | Docker and a browser opener
|===
...
slide decks at `http://localhost:8880/++<++filename++>++`. The `:MarpPreview` command hands this URL to `util.open++_++url`, which picks a browser opener for your platform — `wslview` or PowerShell under WSL, `xdg-open` on Linux, `open` on macOS — and also copies it to the clipboard.
...
|*A browser opener* |Open the preview URL in a browser |`xdg-open` is pre-installed on most Linux desktops; under WSL PowerShell's `Start-Process` is used instead and is always present
