...
[cols="1,3"]
|===
| xref:#how-chosen[How Provider Is Chosen] | Auto-selection logic
| xref:#gui-x11[GUI Linux — X11] | xclip / xsel setup
| xref:#gui-wayland[GUI Linux — Wayland] | wl-clipboard setup
| xref:#wsl[WSL] | win32yank.exe setup
| xref:#osc52[SSH / TTY / Console] | OSC 52 terminal clipboard
| xref:#troubleshooting[Troubleshooting] | Common issues and fixes
|===
