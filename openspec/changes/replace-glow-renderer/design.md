...
- **Does `<localleader>pp` still make sense?** With in-buffer rendering the buffer *is* the preview, so a popup showing the same file is arguably redundant — the honest replacement might be "toggle rendering on this buffer". Kept as a popup here to avoid a spec change the proposal did not ask for, but worth deciding during implementation rather than inheriting by default.
- **Should the cheatsheet float be scrollable-by-default with `nowrap` available on a key?** Only worth answering once the wrapping is seen against real cheatsheet content.
