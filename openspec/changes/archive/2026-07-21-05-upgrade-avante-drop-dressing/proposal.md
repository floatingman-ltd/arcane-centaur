...
- **Simplify to Ollama-only.** Keep the ollama default provider but reduce its model to the small `qwen2.5:0.5b` (~0.4 GB) so it runs on very-limited-RAM machines. **Remove the `claude` provider and its `<leader>ac` keymap** — Anthropic's ToS scopes subscription OAuth tokens to Claude Code / claude.ai (so avante can't safely lean on a subscription), and requiring an `ANTHROPIC_API_KEY` adds an external account/billing dependency this config would rather avoid. Kept keymaps: `<leader>aa` / `<leader>ao`.
...
- `ai-research-ollama`: the default avante ollama model changes to the small **`qwen2.5:0.5b`** (from the spec's `llama3.1:8b` / the prior config's `llama3.2:3b`) for limited-RAM machines, and ollama becomes avante's **only** provider.
...
- **`lua/plugins/avante.lua`** → `version` bumped, `build` updated, `dressing.nvim` removed from `dependencies`, stale pin comment rewritten. `providers` reduced to `ollama` only (model `qwen2.5:0.5b`); the `claude` provider block and `<leader>ac` keymap removed; `<leader>aa`/`<leader>ao` kept. A comment documents re-enabling claude via an API key.
- **`dressing.nvim`** → no longer installed after `:Lazy clean`.
- **`diffview.nvim`** → unaffected, because plenary is retained.
- **No `ANTHROPIC_API_KEY` dependency** — the claude provider is gone; the ollama endpoint (`http://127.0.0.1:11434`) is unchanged, its model is now `qwen2.5:0.5b`.
