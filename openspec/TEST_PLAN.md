...
- [ ] Install **universal-ctags** (`ctags`) — **test-only**, used to generate the `tags` file for Change 06 §6.6 (`]t`/`[t` tag navigation). **Not** a runtime dependency: `]t`/`[t` (vim-unimpaired) only need a `tags` file to exist; the config never invokes `ctags`. `sudo apt install universal-ctags` (Debian/Ubuntu; or `brew install universal-ctags`). Verify: `ctags --version`
