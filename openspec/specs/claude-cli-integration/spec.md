...
## Purpose
In-editor `:ClaudeSuggest` / `:ClaudeExplain` commands that send the current selection (or whole buffer) to the `claude` CLI and show the response in a floating window, using Claude Code's built-in authentication.
