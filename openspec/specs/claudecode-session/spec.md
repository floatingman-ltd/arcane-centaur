...
- **WHEN** the user presses `<leader>aa` or `<leader>ao` (the config's avante maps; the Claude provider map `<leader>ac` is intentionally disabled)
- **THEN** avante SHALL respond exactly as before, and no claudecode / `ClaudeCode` map SHALL occupy the `<leader>a` namespace
