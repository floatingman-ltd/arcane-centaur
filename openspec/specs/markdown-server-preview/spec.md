# markdown-server-preview Specification

## Purpose
Define what the markserv Docker preview server renders, and what it must keep rendering. This is the project-wide browser preview reached by `,sp` / `:MdServerPreview` — a local Node server in `docker/markserv/`, not upstream markserv — and it is distinct from the in-editor paths covered by `markdown-native-rendering` and `markdown-popup-preview`.

It exists because the preview server is the only place some documents can be reviewed whole: it is where relative links between markdown files resolve and where PlantUML and Mermaid diagrams render alongside the text. That makes its rendering contract worth stating rather than leaving to the implementation, particularly the parts that must *not* drift — the diagram fence overrides, live reload, and the plain-blockquote behaviour that anything unrecognised falls back to.
## Requirements
