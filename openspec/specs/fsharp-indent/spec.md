# fsharp-indent Specification

## Purpose
Defines the F# editing behaviour that the editor itself has to supply, because nothing else here can. Indentation is the substance of it: pressing Enter after a construct that opens a body indents that body, and `==`, `>>` and `<<` work as reindent operators.

F# is the only language in this configuration needing a hand-written `indentexpr`. nvim-treesitter ships no `indents.scm` for F# — there is no `queries/fsharp` directory at all — LSP has no indent-as-you-type concept, so `fsautocomplete` cannot help despite supplying folds and formatting, and `smartindent` keys off `{`, `}` and `cinwords`, none of which F# uses.

The implementation is therefore a **vendored** third-party file, `indent/fsharp.vim`, taken from `ionide/Ionide-vim` at a commit recorded in its header, with one function deliberately differing from upstream: comment and string detection, which upstream does with `synID()` and which is inert under treesitter highlighting. That deviation is what stops pair matching treating a delimiter inside a comment or string as a real one.

This capability is separate from `fsharp-lsp` on purpose. Indentation must work with no server attached, with no server installed, and in a standalone `.fsx` whose project options do not resolve — so coupling it to the language server would assert a dependency the tests are meant to disprove.

Comment support belongs here for the same reason: Neovim ships no `ftplugin` for F#, so `commentstring` was unset and commenting failed outright. It is editor-side behaviour no language server provides, found by this capability's own validation.
## Requirements
