# markdown-lsp Specification

## Purpose
Defines markdown language-server support: `marksman` registered through the native `vim.lsp` API with the shared `on_attach`, supplying hover, go-to-definition, references, rename, document symbols, code actions and completion — resolving internal references between documents in the workspace, which is the feature that earns it a place.

Equally, it records the two things marksman does **not** supply, because both are places a reader would reasonably expect it to. It advertises no `foldingRangeProvider`, so markdown folding is unaffected by installing it and continues to come from treesitter with an indent fallback. It advertises no formatting capability either, and no LSP-backed markdown formatter is configured, so writing a markdown buffer never reformats it.
## Requirements
