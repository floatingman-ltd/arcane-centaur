...
Defines buffer-local indent settings for Lua files.

## Requirements

### Requirement: Lua filetype plugin sets 2-space indentation
The config SHALL provide `after/ftplugin/lua.lua` that sets `shiftwidth=2`, `tabstop=2`,
`softtabstop=2`, and `expandtab` for all Lua buffers, matching Neovim's own Lua coding style.
