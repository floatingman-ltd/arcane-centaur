...
Step-by-step tests for every piece of custom tooling in this configuration.
Work through each section in order — earlier sections establish prerequisites
for later ones. Mark steps `[x]` as you go.

**Conventions used below:**
- Commands prefixed `:` are typed in Neovim command mode.
- Commands prefixed `$` are run in a shell.
...
- `nvim` — Neovim 0.11+
- `glow` — Charmbracelet Markdown renderer (see docs/guides/cli-console-mode.md for install)
...
- [ ] 3.1 In a normal GUI/WSL session, confirm `is_console` is false:
  ```
...
- [ ] 4.7 Remove `glow` from PATH temporarily and confirm warning:
  ...
  ,p
  ```
  **Expected:** a `WARN` notification mentioning the install command. No crash.
  Restore: `:lua vim.env.PATH = vim.fn.system("echo -n $PATH")`
...
- [ ] 5.2 Open browser preview:
  ...
  ,p
  ```
  **Expected:** browser tab opens with live-rendered Markdown. No split opens in Neovim.
...
- [ ] 6.2 Open a markdown file and run:
  ...
  ,sp
  ```
  **Expected:** an `INFO` notification (console) or browser tab (GUI) opens at `http://localhost:8090/<file>`.
...
- [ ] 8.1 Run ASCII preview command:
  ...
  :PumlPreviewAscii
  ```
  **Expected:** a **floating popup** (rounded border, centered, ~70% of editor size) opens containing the ASCII diagram.
...
- [ ] 9.1 Run export:
  ...
  ,dp
  ```
  **Expected:** a notification appears; `/tmp/test-diagrams.pdf` is created containing the diagram.
...
- [ ] 10.1 Start MARP preview:
  ...
  ,mp
  ```
  **Expected:** browser opens (GUI) or URL notification (console) at the MARP preview server.
...
- [ ] 11.2 Open with `<leader>aa`. **Expected:** avante chat panel opens, provider is `ollama`.

- [ ] 11.3 Type a question (`What is 2+2?`) and submit (`<CR>`).
  **Expected:** a response streams into the avante buffer. No crash.
...
- [ ] 12.1 Switch to copilot provider and open:
  Press `<leader>ac`.
  **Expected:** avante opens with provider `copilot`.

- [ ] 12.2 Ask a question and submit.
  **Expected:** Copilot response streams in. No separate login prompt.

---

## 13. Copilot Inline Completion

- [ ] 13.1 Open a code file (e.g. a `.lua` or `.py` file) and start typing a function:
  ```lua
  -- In a .lua file
...
- [ ] 13.2 Accept suggestion with `<Tab>`. **Expected:** suggestion is inserted.

- [ ] 13.3 Test Copilot explain (visual mode):
  Select a few lines with `V`, then press `<leader>gce`.
  **Expected:** Copilot provides an explanation in a popup or panel.

---

## 14. LSP

**Precondition:** at least one LSP server is configured (e.g. open a `.lua` file — `lua_ls` if installed, or a `.fs` file for `fsautocomplete`).
...
- [ ] 14.4 Check diagnostics panel:
  ...
  ```
  **Expected:** shows diagnostics for current line (or "No diagnostics" if clean).
...
- [ ] 15.1 Open file finder:
  ...
  <leader>ff
  ```
  **Expected:** FZF picker opens listing files. Type to filter, `<CR>` to open.
...
- [ ] 17.2 Confirm gitsigns decorates changed lines.
  Modify a tracked file and check the sign column.
  **Expected:** `+` / `~` / `-` signs appear in the gutter.
...
## 19. Common Lisp REPL (Conjure + Swank)
...
**Precondition:** `CONFLUENCE_EMAIL` and `CONFLUENCE_API_TOKEN` env vars are set.
...
- [ ] 22.1 Confirm commands are registered:
  ...
  :MdToConfluence
  ```
  **Expected:** either prompts for a page ID / proceeds, or errors with a meaningful message (not `E492: Not an editor command`).
...
**Precondition:** `JIRA_EMAIL`, `JIRA_API_TOKEN`, and `JIRA_BASE_URL` env vars are set.
...
- [ ] 23.2 Run create issue and confirm it prompts for a summary:
  ...
  :JiraCreateIssue
  ```
  **Expected:** an input prompt appears asking for a summary (or errors with a meaningful message if credentials are missing).
...
- [ ] 25.2 Exit terminal insert mode:
  Press `<Esc>` while in the terminal.
  **Expected:** returns to normal mode (cursor visible in terminal buffer) — does **not** close the terminal.
...
- [ ] 26.1 Open a Lisp or F# file with intentionally bad indentation, then save:
  ```
  :w
  ```
...
- [ ] 26.2 Confirm format-on-save is active:
  ...
  ```
  **Expected:** prints the active formatter name (e.g. `lsp_format`).
